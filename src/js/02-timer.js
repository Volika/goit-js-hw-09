...
function updateTime() {
    ...
    const selectedTime = new Date(inputTextEl.value);
...
function updateTimeOnScreen(days, hours, minutes, seconds) {
    outputTime.days.textContent = `${addLeadingZero(days)}`;
    outputTime.hours.textContent = `${addLeadingZero(hours)}`;
    outputTime.minutes.textContent = `${addLeadingZero(minutes)}`;
    outputTime.seconds.textContent = `${addLeadingZero(seconds)}`;
}
